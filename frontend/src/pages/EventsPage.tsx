...
const EventsPage = () => {
  const { items, loading } = useSectionItems("events");

  const events = items.map(item => ({
    date: item.startDate || "TBD",
    time: "TBA", 
    title: item.title,
    desc: item.description || "",
    type: item.subtitle || "Event"
  }));
  ...
  return (
    <PageLayout title="Event Calendar" subtitle="Join our upcoming conferences, workshops, and high-impact academic events building the future.">
      {loading ? (
        <div className="text-center py-12"><p className="text-slate-500">Loading events...</p></div>
      ) : events.length === 0 ? (
        <div className="text-center py-12"><p className="text-slate-500 text-lg">No upcoming events scheduled.</p></div>
      ) : (
        <motion.div 
